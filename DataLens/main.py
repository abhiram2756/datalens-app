...
def _correlation_heatmap(ax, df, cols):
    ...
    sns.heatmap(corr_clean, ax=ax, annot=True, fmt=".2f", cmap=orange_cmap,
                linewidths=0.5, linecolor="#0A0A0A",
                annot_kws={"size": 9, "color": "#111111", "weight": "bold"},
                vmin=0, vmax=1,
                cbar_kws={"shrink": 0.8, "label": "Correlation Strength"})
    ax.set_title("Correlation Heatmap  (darker orange = stronger)", color=TEXT_COLOR, pad=14)
    ax.tick_params(axis="x", rotation=35, labelsize=9, colors="#CCCCCC")
    ax.tick_params(axis="y", rotation=0,  labelsize=9, colors="#CCCCCC")
...
def generate_charts(df):
    ...
    if len(numeric_cols) >= 2:
        hmap_cols = numeric_cols[:12]
        sz = max(9, len(hmap_cols))
        ...
        chart_groups.append({
            ...
            "charts": [make_chart(
                "Correlation Heatmap",
                "Darker orange = stronger correlation between two columns.",
    ...
    # ── 6. Pie charts ────────────────────────────────────────────────────────
    pie_cols = [c for c in cat_cols if 2 <= df[c].nunique() <= 7]
    if pie_cols:
        pie_cards = []
        for gi, col in enumerate(pie_cols):
            fig, ax = plt.subplots(figsize=(8, 7), facecolor=BG_COLOR)
            ax.set_facecolor(BG_COLOR)
            _pie(ax, df[col])
            plt.tight_layout(pad=1.5)
            path = f"static/chart_pie_{gi}.png"
            _save(fig, path)
            pie_cards.append(make_chart(
                f"Composition – {col}",
                "Each slice = one category value. Percentage shown inside.",
                path, {col: col_colors[col]}
            ))
        chart_groups.append({
            "label": "Pie Charts",
